...
function checkRemotePushImplementation() {
  ...
  if (!backendFile || !existsSync(backendFile)) {
    ...
    const socialGatewayMarkers = [
      "deliverGatewayGroup",
      "purgeExpiredNotificationPushers",
      "isNotificationGatewayAllowed",
      '"event_id_only"',
      ".delete(notificationPushers)",
    ];
    const selectedMarkers = backendSource.includes("deliverGatewayGroup")
      ? socialGatewayMarkers
      : takosGatewayMarkers;
    const missingMarkers = selectedMarkers.filter(
      (marker) => !backendSource.includes(marker),
    );
    facts.remotePush.deliveryBackend = backendSource.includes(
      "deliverGatewayGroup",
    )
      ? "yurucommu.notification-pusher-gateway.v1"
      : "takos.notification-pusher-gateway.v1";
