...
// Plugins whose crate ships a default permission set that grants commands but
// no scope, so a capability that only lists bare identifiers hands the WebView
// an unrestricted surface. Each entry is justified against the crate's
// permissions/*.toml, and lists the permissions that carry their own scope.
const scopeRequiringPlugins = {
  // tauri-plugin-http permissions/default.toml: "This enables all fetch
  // operations but does not allow explicitly any origins to be fetched. This
  // needs to be manually configured before usage." Every autogenerated
  // allow-fetch* permission is "without any pre-configured scope", so nothing
  // in the plugin supplies one.
  http: [],
  // tauri-plugin-opener autogenerated open_url.toml / open_path.toml /
  // reveal_item_in_dir.toml are all "without any pre-configured scope"; only
  // permissions/allow-default-urls.toml declares [[permission.scope.allow]]
  // entries (mailto:, tel:, http://, https://).
  opener: ["allow-default-urls"],
};

// Bridge call sites whose plugin command falls outside the plugin's default
// permission set, so the capability has to name the permission explicitly.
// Detection is on mobile-kit's own bridge source: if the call site is reachable
// the permission is mandatory, and if it is deleted the entry stops firing.
const bridgeReachableCommands = [
  {
    // Stronghold's permissions/default.toml grants only "non-destructive
    // operations"; Store.remove() invokes plugin:stronghold|remove_store_record,
    // which the default set omits, so seed cleanup silently fails without it.
    marker: "current.store.remove(",
    permission: "stronghold:allow-remove-store-record",
    capabilities: ["default", "mobile"],
  },
  {
    // The shells grant barcode-scanner permissions individually rather than
    // barcode-scanner:default, so the camera permission gate the bridge runs
    // before scanning needs both commands named.
    marker: "barcodeScanner.checkPermissions()",
    permission: "barcode-scanner:allow-check-permissions",
    capabilities: ["mobile"],
  },
  {
    marker: "barcodeScanner.requestPermissions()",
    permission: "barcode-scanner:allow-request-permissions",
    capabilities: ["mobile"],
  },
  {
    // opener:allow-default-urls scopes open_url but does not enable the command.
    marker: "options.opener.openUrl(",
...
checkFile("src-tauri/.cargo/config.toml");
checkIconFiles();
...
const cargoConfigToml = readText("src-tauri/.cargo/config.toml");
const mobileKitTauriBridgeTs = readFileSync(
  new URL("../src/tauri-bridge.ts", import.meta.url),
  "utf8",
);
const mobileKitNativeInitMjs = readFileSync(
  new URL("./init-tauri-mobile-native.mjs", import.meta.url),
  "utf8",
);
...
checkCapabilityScopes(defaultCapability, mobileCapability);
checkCapabilityPluginCoverage(cargoToml, defaultCapability, mobileCapability);
checkBridgeReachableCommands(mobileKitTauriBridgeTs, {
  defaultCapability,
  mobileCapability,
});
checkAndroidNativePatchWiring(cargoConfigToml, packageJson);
...
function checkTauriConfig(config) {
  ...
  // Tauri's own default is API 24, which predates the scoped-storage and
  // per-app-signing-key guarantees the keystore plugin relies on.
  expect(
    typeof config.bundle?.android?.minSdkVersion === "number" &&
      config.bundle.android.minSdkVersion >= 28,
    "tauri.conf pins bundle.android.minSdkVersion to 28 or newer",
  );
  ...
  for (const [label, capability] of [
    ...
    for (const entry of entries) {
      ...
      expect(
        ...
        `${label} capability scope for ${entry.identifier} is non-empty`,
      );
    }
    for (const [plugin, selfScoped] of Object.entries(scopeRequiringPlugins)) {
      const granted = entries.filter((entry) => entry.plugin === plugin);
      if (granted.length === 0) continue;
      // Tauri merges every permission a capability grants for one plugin into a
      // single scope, so one scoped entry covers the plugin's bare identifiers.
      const scoped = granted.some(
        (entry) =>
          (entry.scoped && Array.isArray(entry.allow) && entry.allow.length) ||
          selfScoped.includes(entry.identifier.slice(plugin.length + 1)),
      );
      expect(
        scoped,
        `${label} capability restricts ${plugin} permissions to an explicit scope`,
      );
    }
  }
}

function checkCapabilityPluginCoverage(
  cargoToml,
  defaultCapability,
  mobileCapability,
) {
  const dependencies = cargoPluginDependencies(cargoToml);
  const defaultEntries = capabilityPermissionEntries(defaultCapability);
  const mobileEntries = capabilityPermissionEntries(mobileCapability);
  ...
  for (const dependency of dependencies) {
    ...
    expect(
      defaultPlugins.has(dependency.plugin),
      `desktop capability grants a permission for ${dependency.plugin}`,
    );
    expect(
      mobilePlugins.has(dependency.plugin) ||
        defaultPlugins.has(dependency.plugin),
      `mobile platforms keep a permission for ${dependency.plugin}`,
    );
  }
  ...
  for (const [label, plugins] of [
    ...
    for (const plugin of plugins) {
      ...
      expect(
        declared.has(plugin),
        `${label} capability grants ${plugin} only because Cargo.toml depends on it`,
      );
    }
  }
}

function checkBridgeReachableCommands(
  mobileKitTauriBridgeTs,
  { defaultCapability, mobileCapability },
) {
  const identifiers = {
    default: new Set(
      capabilityPermissionEntries(defaultCapability).map(
        (entry) => entry.identifier,
      ),
    ),
    mobile: new Set(
      capabilityPermissionEntries(mobileCapability).map(
        (entry) => entry.identifier,
      ),
    ),
  };
...
function checkAndroidNativePatchWiring(cargoConfigToml, packageJson) {
  // Android 16 runs the 64-bit ABIs on 16 KB pages, and the NDK only injects
  // the linker flag through its own CMake / ndk-build front ends, which the
  // Rust JNI library never goes through.
  const cargoConfigSections = tomlSections(cargoConfigToml);
  for (const target of ["aarch64-linux-android", "x86_64-linux-android"]) {
    expect(
      (cargoConfigSections[`target.${target}`] ?? "").includes(
        "max-page-size=16384",
      ),
      `cargo config pins 16 KB page alignment for ${target}`,
    );
  }

  // src-tauri/gen is regenerated and gitignored, so the manifest hardening has
  // to stay a repeatable step that native init re-applies.
  expectScript(packageJson, "tauri:android-manifest:apply");
  expectScript(packageJson, "tauri:android-manifest:verify");
  expect(
    packageJson.scripts?.["tauri:android-manifest:verify"]?.includes(
      "--dry-run",
    ) &&
      packageJson.scripts?.["tauri:android-manifest:verify"]?.includes(
        "--strict",
      ),
    "script tauri:android-manifest:verify fails on unapplied manifest patches",
  );
  expect(
    mobileKitNativeInitMjs.includes("apply-tauri-mobile-android-manifest.mjs"),
    "native init re-applies the Android manifest patch after gen regeneration",
  );
}

function capabilityPermissionEntries(capability) {
  return (capability.permissions ?? []).map((entry) => {
    ...
    return {
      ...
      plugin: typeof identifier === "string" ? identifier.split(":")[0] : "",
      scoped: typeof entry !== "string",
      allow: typeof entry === "string" ? undefined : entry?.allow,
    };
  });
}
...
function tomlSections(text) {
  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (name) sections[name] += `${rawLine}\n`;
  }
  return sections;
}

function cargoPluginDependencies(cargoToml) {
  const dependencies = [];
  let section = "";
  for (const rawLine of cargoToml.split("\n")) {
    ...
    const match = /^tauri-plugin-([a-z0-9-]+)\s*=/.exec(line);
    if (!match) continue;
    dependencies.push({
      plugin: match[1],
      mobileOnly: section.includes('target_os = "android"'),
    });
  }
  return dependencies;
}
...
function checkIosPlist(iosPlist) {
  ...
  // App Review rejects a usage key whose string is empty, and the OS still
  // terminates the app the moment the API is touched.
  for (const key of ["NSCameraUsageDescription", "NSFaceIDUsageDescription"]) {
    if (!iosPlist.includes(key)) continue;
    expect(
      new RegExp(
        `<key>\\s*${key}\\s*</key>\\s*<string>\\s*\\S[^<]*</string>`,
      ).test(iosPlist),
      `iOS ${key} has a non-empty purpose string`,
    );
  }
}
...
function checkRemotePushPlugin(
  ...
  expect(
    nativeTs.includes('from "./mobile-push.ts"') &&
