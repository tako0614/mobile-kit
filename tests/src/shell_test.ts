...
import { defineMobileHostActions } from "../../src/host-actions.ts";
import { resolvePushNotificationPath } from "../../src/push-navigation.ts";
...
const adapter: MobileProductAdapter = {
  ...
  directDeployUrl: "https://deploy.example/?source=takos",
...
test("createFirstRunActions can hide Host Center action for Host Center-only adapters", () => {
  ...
  ).toEqual(["url", "qr", "direct-deploy"]);
});

test("createDirectDeployHref requires an explicitly secure product URL", () => {
  expect(createDirectDeployHref(adapter)).toBe(
    "https://deploy.example/?source=takos",
  );
  expect(() =>
    createDirectDeployHref({
      ...adapter,
      directDeployUrl: "http://deploy.example/takos",
    }),
  ).toThrow("Direct deployment URL must use https");
});
