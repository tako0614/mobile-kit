...
test("mobile client revokes an advertised host bearer session on sign out", async () => {
  ...
  await bridge.storage?.set(
    ...
    JSON.stringify({
      hostUrl: "https://host.example",
      product: "takos",
      productEndpoints: { mobileLogout: "/api/auth/logout" },
  ...
  expect(requests).toEqual([
    {
      url: "https://host.example/api/auth/logout",
...
test("mobile client clears its local session when host revocation fails", async () => {
  const bridge = memoryBridge();
  await bridge.storage?.set(
    mobileSessionStorageKey(adapter),
    JSON.stringify({
...
test("mobile client controller opens the product direct deployment flow", async () => {
  const bridge = memoryBridge();
  const controller = createMobileClientController({
    adapter,
    nativeBridge: bridge,
  ...
  await controller.selectAction("direct-deploy");

  expect(bridge.opened).toEqual(["https://deploy.example/?source=takos"]);
});
