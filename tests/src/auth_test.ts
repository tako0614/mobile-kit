...
test("completeMobileOidcSignIn exchanges the IdP token for a host session when advertised", async () => {
  ...
  const fetcher: FetchLike = async (input, init) => {
    ...
    if (url.endsWith("/api/auth/mobile/oidc")) {
    ...
    return oidcFetchWithIdToken()(input, init);
  };
  await beginMobileOidcSignIn({
    adapter,
    discovery: {
      ...
      product: {
        product: "takos",
        endpoints: { mobileOidcExchange: "/api/auth/mobile/oidc" },
      },
    },
    nativeBridge: bridge,
    fetch: fetcher,
  ...
  expect(session.accessToken).toBe("host-session");
  expect(session.refreshToken).toBeUndefined();
  expect(session.expiresAt).toBe("2026-07-16T02:00:00.000Z");
});

test("signInWithMobilePassword stores the host session", async () => {
  const bridge = memoryBridge();
  ...
  expect(session.accessToken).toBe("password-session");
  expect(
    (await loadMobileSession({ adapter, nativeBridge: bridge }))?.accessToken,
  ).toBe("password-session");
});

test("signInWithMobilePassword preserves password whitespace", async () => {
  const bridge = memoryBridge();
  await signInWithMobilePassword({
    adapter,
    discovery: {
      hostUrl: "https://host.example",
      authMethods: { oidc: false, password: true },
...
test("expired host sessions without refresh tokens require sign-in again", async () => {
  await expect(
    ensureFreshMobileSession({
      adapter,
      nativeBridge: memoryBridge(),
      session: {
        hostUrl: "https://host.example",
        product: "takos",
        accessToken: "expired-host-session",
        tokenType: "Bearer",
  ...
  ).rejects.toThrow("Mobile session has no refresh token.");
});
...
function oidcFetchWithIdToken(): FetchLike {
  const base = oidcFetch();
  return async (input, init) => {
    const response = await base(input, init);
    if (!String(input).endsWith("/oauth/token") || !response.ok)
      return response;
