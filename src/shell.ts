...
export function createFirstRunActions(
  ...
  if (adapter.directDeployLabel && adapter.directDeployUrl) {
    actions.push({
      id: "direct-deploy",
      label: adapter.directDeployLabel,
      description:
        adapter.directDeployDescription ??
        `Create a new ${adapter.hostNoun} with the direct deployment provider.`,
    });
  }
  if (
...
export function createDirectDeployHref(adapter: MobileProductAdapter): string {
  if (!adapter.directDeployLabel || !adapter.directDeployUrl) {
    throw new Error("Direct deployment action is not configured for this app.");
  }
  return requireSecureWebUrl(
    adapter.directDeployUrl,
...
export async function copyMobileText(
  input: CopyMobileTextInput,
): Promise<void> {
